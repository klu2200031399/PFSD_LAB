...
def add():
    ...
    new_data = {
        ...
        'No. of year Experience': experience,  # Corrected field name
...
@app.route('/delete/<int:index>', methods=['POST', 'DELETE'])
def delete(index):
    if request.method == 'DELETE':
        current_data = read_csv('Employee.csv')
        del current_data[index - 1]  # Adjust index to match Python indexing (starting from 0)
        write_csv('Employee.csv', current_data)
        return redirect(url_for('index'))
